...
Why it matters: decoupling interface from implementation allows you to swap representations without breaking any code that uses the type. This is the foundation of modular, maintainable software.
...
_Reference: Lippman, Lajoie & Moo (2013), C++ Primer, 5th ed.; Meyers (2014), Effective Modern C++_
...
To support move operations, C++ introduced **rvalue references**, written with `&&` instead of `&`. An rvalue reference can only be bound to an rvalue: a literal, or the result of a function or operator call that does not return a reference.

```cpp
#define rvalue 42
int i = rvalue;         // use rvalue to initialise i
int&& r1 = rvalue;      // rvalue reference to a literal
int&& r2 = i * rvalue;  // rvalue reference to result of multiplication
```

The key insight: rvalue references bind only to objects whose lifetime is about to expire. This is the signal to steal their resources rather than copy them.
...
**Template argument deduction** is the process by which the C++ compiler infers the type parameters of a function template from the types of the arguments passed at the call site. You do not need to explicitly specify `<int>` when calling a template that receives an `int`; the compiler figures it out.
...
`std::forward<U>(arg)` preserves the value category of an argument when passing it deeper into a call chain. It is also a compile-time-only cast that generates no executable code:

- If the original argument was an rvalue, `std::forward` returns an rvalue reference.
- If the original argument was an lvalue, `std::forward` returns it unchanged as an lvalue reference.
...
{
    f( p );                        // always [l-value]: p has a name
    f( std::forward<T>( p ) );    // preserves original category
}
...
Without `std::forward`, every argument forwarded through a named parameter loses its rvalue nature. This matters for containers like `emplace()`, which must construct elements using the caller's original argument types.
...
_Reference: Cormen et al. (2022), Introduction to Algorithms, 4th ed., Chapter 10_
...
`top()` returns the oldest element (the front of the queue) without removing it. It returns `std::optional<T>` so that the check for emptiness and the retrieval of the value happen as a single atomic operation, which is essential for thread-safe code. The running time is $O(1)$.

`enqueue(value)` inserts a new element at the tail of the queue. The tail advances in round-robin fashion over the backing array. The running time is $O(1)$.

`dequeue()` removes the oldest element by advancing the head pointer in round-robin fashion. The slot is not immediately zeroed; it will be overwritten on the next enqueue. The running time is $O(1)$.
